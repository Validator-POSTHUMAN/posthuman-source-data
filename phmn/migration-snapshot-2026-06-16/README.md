...
- `phmn_final_distribution_broadcast_candidate_breakdown.csv` - human-readable
  public review table with `recipient_address,source_addresses,amount_phmn,note`.
  ...
  address or addresses produced the recipient row. The `note` column is blank
  for normal user rows and explains the large SubDAO treasury rows.
