...
- `phmn_final_distribution_broadcast_candidate_breakdown.csv` - human-readable
  public review table with `recipient_address,amount_phmn,note`. Amounts are
  shown in PHMN with 6 decimal places, without micro units or denom placeholders.
  The `note` column is blank for normal user rows and explains the large
  SubDAO treasury rows.
