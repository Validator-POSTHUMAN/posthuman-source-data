...
- `phmn_final_distribution_broadcast_candidate_breakdown.csv` - human-readable
  public review table with `source_addresses,recipient_address,amount_phmn,note`.
  Amounts are shown in PHMN with 6 decimal places, without micro units or denom
  placeholders. The `source_addresses` column shows which old PHMN source
  address or addresses produced the recipient row. The `note` column explains
  rows with multiple source addresses, all allocations above 100 PHMN, and the
  large SubDAO treasury rows.
