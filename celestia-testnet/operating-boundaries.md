# Celestia Mocha-5 — Operating Boundaries

Use this page to choose the correct Node Ops guide and to keep public evidence
separate from operating authority. It does not enable a service, expose an
endpoint, register a Fibre provider, or send a transaction.

## Choose the correct role

`celestia-appd` is the Mocha-5 consensus full-node and validator plane.
`celestia-node` is the DA plane. The supported DA roles are **Bridge** and
...
- DA role selection and Mocha-4 isolation: [Node roles](full-node-setup.md).
- Bridge or Light service: [DA bridge](bridge-node-setup.md) or
  [DA light](light-node-setup.md).
- Validator-side Fibre provider after app v10: [Fibre](fibre.md).

Mocha-5 is a distinct chain. Never reuse Mocha-4 consensus data, DA stores, or
signer state.
...
The [Bridge Nodes Explorer](/explorer/celestia-testnet/bridge-nodes) is a public
...
templates are not runnable for Mocha-5.
...
before provisioning. Correlate local evidence with the [official Mocha status
history](https://status.celestia.dev/status/mocha); neither a green status page
nor host reachability proves that a local node is correct.

For alerting, Fibre registration completion, and recovery evidence, use
[Monitoring](monitoring.md) and [Fibre](fibre.md).
