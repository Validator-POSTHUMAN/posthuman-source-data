...
| Required release after height 7,725,000 | `v10.1.0` |
| Binary | `exrpd` |
| Home | `/var/lib/exrpd/.exrpd` |

Check the [official network page](https://docs.xrplevm.org/pages/operators/resources/networks)
and the [official releases](https://github.com/xrplevm/node/releases) before
installation.
...
sudo -u exrpd curl -fsSL \
  https://raw.githubusercontent.com/xrplevm/networks/main/testnet/genesis.json \
  -o /var/lib/exrpd/.exrpd/config/genesis.json

PEERS="$(curl -fsSL https://raw.githubusercontent.com/xrplevm/networks/main/testnet/peers.txt \
...
sudo sed -i \
  's/^evm-chain-id *=.*/evm-chain-id = 1449000/' \
