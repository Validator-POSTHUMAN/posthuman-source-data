...
LOCK_FILE="/tmp/posthuman-celestia-testnet-decentralize-map-update.lock"
MAP_FILE="celestia-testnet/decentralize-map.json"
...
if git diff --quiet -- "$MAP_FILE"; then
  echo "No Celestia testnet decentralize-map changes."
  exit 0
fi
