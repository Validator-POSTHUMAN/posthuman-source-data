...
- Latest archive:
  https://snapshots-gnoland.posthuman.digital/gnoland_topaz-1-latest.tar.lz4
- SHA-256:
...
```bash
curl -fL --retry 3 \
  -o "$TMP_DIR/gnoland_topaz-1-latest.tar.lz4" \
  "$SNAPSHOT_URL/gnoland_topaz-1-latest.tar.lz4"
curl -fL --retry 3 \
...
lz4 -t gnoland_topaz-1-latest.tar.lz4
if lz4 -dc gnoland_topaz-1-latest.tar.lz4 \
  | tar -tf - \
...
lz4 -dc "$TMP_DIR/gnoland_topaz-1-latest.tar.lz4" \
  | tar -xf - -C "$DATA_DIR"
