# Gnoland Pearl Snapshot

POSTHUMAN publishes a verified LZ4 snapshot from a dedicated non-signing
Pearl RPC node.

- Latest archive:
  https://snapshots-gnoland.posthuman.digital/gnoland_pearl-1-latest.tar.lz4
- SHA-256:
...
```bash
curl -fL --retry 3 \
  -o "$TMP_DIR/gnoland_pearl-1-latest.tar.lz4" \
  "$SNAPSHOT_URL/gnoland_pearl-1-latest.tar.lz4"
curl -fL --retry 3 \
...
lz4 -t gnoland_pearl-1-latest.tar.lz4
if lz4 -dc gnoland_pearl-1-latest.tar.lz4 \
  | tar -tf - \
...
lz4 -dc "$TMP_DIR/gnoland_pearl-1-latest.tar.lz4" \
  | tar -xf - -C "$DATA_DIR"
