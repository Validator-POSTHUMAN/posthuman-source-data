...
## What it adds on DevNet

DevNet and TestNet are reset roughly every three months. The skill treats a
...
fresh onboarding secret — self-service on DevNet — redeploy on the announced
migration ID, and take a **new** identities backup, because identities change
as part of the reset. It explicitly refuses to "fix" a post-reset node by
upgrading its image.

DevNet is also where the skill's Scan-reading steps can be exercised without a
whitelist, because DevNet Scan is public. The same code paths then work
unchanged against a whitelisted MainNet host.
