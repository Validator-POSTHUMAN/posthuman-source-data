# Monad Testnet Node Installation Guide
...
````bash
sudo apt update
sudo apt install -y monad=0.12.7
...
sudo curl -o /home/monad/.env $MF_BUCKET/config/testnet/latest/.env.example
sudo curl -o /home/monad/monad-bft/config/node.toml $MF_BUCKET/config/testnet/latest/full-node-node.toml
````
...
MF_BUCKET=https://bucket.monadinfra.com
sudo curl -o /home/monad/.env $MF_BUCKET/config/testnet/latest/.env.example
sudo curl -o /home/monad/monad-bft/config/node.toml $MF_BUCKET/config/testnet/latest/node.toml
````
...
Create a monitoring script that alerts on service failures, sync stalls, and resource issues:
...
Replace `YOUR_BOT_TOKEN` and `YOUR_CHAT_ID`, then set up a systemd timer:
...
Alerts trigger **only on state changes** — no spam.
